...
function Policy({ policy, query }) {
  ...
  return (
    <li key={policy.id}>
      <h3>{policy.title}</h3>
      <Link to={relevantReqs}>{policy.relevant_reqs}</Link> of <Link to={allReqs}>{policy.total_reqs}</Link> match your search
      <span className="download-policy-link">Download policy PDF</span>
      <span className="view-requirements-link"><Link to={allReqs}>View all requirements</Link></span>
    </li>
  );
}
